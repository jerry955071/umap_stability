...
seurat_obj <- SCTransform(
  ...
  vars.to.regress = c("S.Score", "G2M.Score", "Protoplasting_1")
)
VariableFeaturePlot(seurat_obj)
```
...
```{r}
# get marker genes for ear FM-S2 from Supplementary data (Sheet9)
markers.sheet9 <- read.csv("data/zma/Sheet9_ear_FM-S2 marker.tsv", sep = "\t")
named_markers <- markers.sheet9 %>% 
  subset(., !markers.sheet9$gene.for.identity.in.Fig..4 %in% c("*", "")) %>% 
  dplyr::group_by(geneID) %>%
  dplyr::summarise(
    clusters = paste(cluster, collapse = ", "),
    gene_name = head(gene.for.identity.in.Fig..4, 1)
  ) %>% 
...
# dot plot
DotPlot(
  seurat_obj, features = named_markers$geneID %>% rev
) + ggplot2::coord_flip()
```
